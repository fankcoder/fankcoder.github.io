...
#### 插入排序法（ Insertion Sort ）
    ...
    从表面上来看是递增法与枚举法：第一层是递增法，逐一把每个数字插入到左方已排序的阵列。第二层是枚举法，搜索插入位置；再将大量数字往右挪，以腾出空间插入数字。
...
#### 二分搜索法（ Binary Search ）
这是在已排序阵列里面搜索数字的方法。阵列由中央切成两边，一边数字较小、一边数字较大。这两边一定有一边不是我们所要的，可以去除，只需要继续寻找其中一边。
